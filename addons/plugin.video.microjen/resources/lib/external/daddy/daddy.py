...
FANART = 'https://raw.githubusercontent.com/thecrewwh/zips/master/matrix/_zip/plugin.video.daddylive/fanart.jpg' or addon.getAddonInfo('fanart')
ICON = 'https://raw.githubusercontent.com/thecrewwh/zips/master/matrix/_zip/plugin.video.daddylive/icon.png' or addon.getAddonInfo('icon')
def build_url(query):
    return base_url + '?' + urlencode(query)

def Main_Menu():
    menu=[
        ['Ζωντανοί αγώνες','sched'],
        ['Ζωντανά Κανάλια','live_tv'],
    ]
    for m in menu:
...
def getSource(trData):
    ...
    select = xbmcgui.Dialog().select('Πηγές', data[2])
    if select > -1:
