    ...
    if 'kanal' in url:
        ...
        for chan in channels:
            if kateg.lower() in chan.lower():
                for href,imag,title in re.findall('href="([^"]+)".*><.*?src="([^"]+)" alt="([^"]+)">', chan):
...
def GetVid(url):
    ...
    if iframe:
        ...
        if playstream:
            ...
            if 'ragnaru.net' in html:
            ...
            elif 'sportcast.life' in html:#2022-08
                headers.update({'referer': playstream_url})
                ...
                if 'daddylive' in url2:
                    ...
                    video_url=re.findall('source:"([^"]+)"',html2, re.DOTALL)[-1]+'|User-Agent={ua}&Referer={ref}'.format(ref=url2,ua=UA)
